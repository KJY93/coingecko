...
async def test_poll_market_data(sample_market_data, mocker):
    ...
    mock_insert = mocker.patch("app.scheduler.jobs.insert_many_market_data")
    mocker.patch("app.scheduler.jobs.asyncio.sleep")

    await poll_market_data()

    assert mock_fetch.call_count == 5
    mock_insert.assert_called_once()
    records = mock_insert.call_args.args[0]
    assert len(records) == 5
    assert records[0].coingecko_id == "bitcoin"
